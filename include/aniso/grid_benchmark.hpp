...
struct GridMetrics {
    ...
    double param1 = 0, param2 = 0;  // swept parameter values (for 2D)
    double avg_x    = 0;
    double avg_E    = 0;
    double avg_trG  = 0;
    double max_aniso = 0;
...
GridMetrics run_grid_one(const YAML::Node& cfg,
    ...
    constexpr double EARLY_STOP_THRESH = 1.0;
    ...
    for (int step = 0; step < n_steps; ++step) {
        ...
        double frame_aniso = 0;
        for (int i = 0; i < Nx; ++i)
            for (int j = 0; j < Ny; ++j) {
                frame_x  += grid.x(i, j).norm();
                frame_E  += grid.E(i, j);
                frame_tr += grid.G(i, j).trace();
                frame_u  += grid.last_u(i, j).norm();
                frame_aniso = std::max(frame_aniso, grid.anisotropy(i, j));
            }
        frame_x  /= total;
        frame_E  /= total;
        frame_tr /= total;
        ...
        peak_aniso = std::max(peak_aniso, frame_aniso);
        ++n_accum;

        if (in_tail) { sum_tail_x += frame_x; ++n_tail; }
    ...
    if (n_tail > 0) m.tail_x = sum_tail_x / n_tail;
    m.breakdown = early_stop || (m.tail_x > 0.4);
    return m;
}
...
bool dump_grid_sweep_csv(const std::vector<GridSweepRow<Dim>>& rows,
    ...
    f << param_name
      << ",controller,avg_x,avg_E,avg_trG,max_aniso,avg_effort,"
         "tail_x,breakdown,wall_ms\n";
    for (auto& row : rows)
        for (auto& m : row.results)
            f << row.param_value << ',' << m.ctrl_name << ','
              << m.avg_x << ',' << m.avg_E << ',' << m.avg_trG << ','
              << m.max_aniso << ',' << m.avg_effort << ','
              << m.tail_x << ',' << (m.breakdown ? 1 : 0) << ','
              << m.wall_ms << '\n';
    return true;
}
